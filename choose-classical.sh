...
sane "$mov_q"
[[ "$mov_q" -gt 1 ]] && {
    chosen_mov="$(cat "$composer_file" <(echo -n "$chosen_title — 000–All in order") | grep "$chosen_title" | awk -F ' — ' '{print $2}' | sort-roman | sort -h | sed 's/^.*–//g' | rofi -normalize-match -dmenu -no-custom -i -p "$chosen_title")"
