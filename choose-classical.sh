...
sane "$mov_q"
[[ "$mov_q" -gt 1 ]] && {
    mvt_file="/tmp/choose-mvt-$(date-string)-$(random-string)"
    cat "$composer_file" > "$mvt_file"
    echo "$chosen_title — 000000000–All in order" >> "$mvt_file"
    grep -q "^$chosen_title$" "$composer_file" && {
	echo "$chosen_title — 000000000000–Full piece" >> "$mvt_file"
    }
    chosen_mov="$(cat "$mvt_file" | grep "^$chosen_title —" | awk -F ' — ' '{print $2}' | sort-roman | sort -h | sort -V | sed 's/^.*–//g' | rofi -normalize-match -dmenu -no-custom -i -p "$chosen_title")"
